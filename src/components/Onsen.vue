          ...
          <v-col cols="12" sm="9" class="onsen__contents-main">
            ...
            <v-row>
              <v-col cols="12" sm="6" v-for="(article) in articles" :key="article.id" align="left">
                <router-link :to="{ name: 'article-detail', params: { id: article.id } }" class="onsen__router-link">
                  <v-row class="onsen__content-side">
                    <v-col cols="5" class="onsen__content-side-frame">
                        <v-img :src="article.imgUrl1" class="onsen__content-side-img">
                          <div class="onsen__content-side-category">
                            <app-chip :text="article.categoryName"></app-chip>
                          </div>
                        </v-img>
                    </v-col>
                    <v-col cols="7" class="onsen__content-side-text">
                      <p class="onsen__content-side-created-at">
                        {{ article.createdAt }}
                      </p>
                      <h4>{{ article.title }}</h4>
                      <p class="onsen__content-side-summary">{{ article.summary }}</p>
                    </v-col>
                  </v-row>
                  <v-divider></v-divider>
                </router-link>
...
.onsen__content-main--flex {
  display: flex;
  padding: 0px 10px 10px 10px;
  height: 150px;
  width: 450px;
  .onsen__content-create-at {
    font-size: 12px;
    margin: 0;
  }                                                                               
  .onsen__content-summary {
    height: 50px;
    overflow: hidden;
